...
try:
    ...
    st.error(
        "Missing dependency: PyMySQL is not installed. Add "
        "`pymysql>=1.1.1` to `requirements.txt`, then redeploy or reboot the app."
    )
    st.stop()
...
Return concise markdown, under 900 words, with these sections:
1. Executive summary
...
- Prioritize the top 10 findings if the scan contains many issues.
- Include at most 3 validation SQL examples.
...
def supports_reasoning_options(model):
...
def extract_response_text(response):
    ...
    for item in getattr(response, "output", []) or []:
        for content in getattr(item, "content", []) or []:
            ...
            if text:
                chunks.append(text)
            elif isinstance(content, Mapping) and content.get("text"):
...
def incomplete_response_message(response):
    ...
    if reason == "max_output_tokens":
        return (
            "AI response stopped before producing advisor text because the token limit "
            "was reached. Try increasing the AI response token limit, lowering the "
            "number of issues sent to AI, or using a smaller scan scope."
        )

    if reason:
        return f"AI response was incomplete: {reason}."

    return "AI returned no advisor text. Try increasing the token limit or reducing the scan scope."
...
def generate_ai_advice(
    ...
    input_text = (
        ...
        "Be direct and concise. Do not describe your reasoning process.\n\n"
    ...
    if supports_reasoning_options(model):
        request["reasoning"] = {"effort": "low"}
        request["text"] = {"verbosity": "low"}

    response = client.responses.create(**request)
    advice = extract_response_text(response)
    if advice:
        return advice

    return incomplete_response_message(response)
...
host = st.sidebar.text_input("Host", type="password")
port = st.sidebar.text_input("Port", "3306")
user = st.sidebar.text_input("User", type="password")
password = st.sidebar.text_input("Password", type="password")
database = st.sidebar.text_input("Database", type="password")
